...
class TestColdstartToGaia(TestCase):
    ...
    def test_entity_translation(self) -> None:
        ...
        ref_graph.add((type_assertion, AIDA.system, system_node))

        # canonical string
        ref_graph.add((entity_node, SKOS.prefLabel, Literal("Hollande Heads")))

        # note what system the entity itself came from
        ref_graph.add((entity_node, AIDA.system, system_node))

        # add justifications
        first_justification = node_gen.next_node()
        ref_graph.add((entity_node, AIDA.justifiedBy, first_justification))
        ref_graph.add((first_justification, RDF.type, AIDA.TextProvenance))
        ref_graph.add((first_justification, AIDA.source, Literal("VOA_EN_NW_2015.11.26.3074625")))
        ref_graph.add((first_justification, AIDA.startOffset, Literal(0)))
        # TODO check inclusive/exclusive
        ref_graph.add((first_justification, AIDA.endOffsetInclusive, Literal(13)))
        ref_graph.add((first_justification, AIDA.system, system_node))

        first_confidence_node = node_gen.next_node()
        ref_graph.add((first_confidence_node, AIDA.confidenceValue, Literal(0.875)))
        ref_graph.add((first_confidence_node, AIDA.system, system_node))
        ref_graph.add((first_justification, AIDA.confidence, first_confidence_node))

        second_justification = node_gen.next_node()
        ref_graph.add((entity_node, AIDA.justifiedBy, second_justification))
        ref_graph.add((second_justification, RDF.type, AIDA.TextProvenance))
        ref_graph.add((second_justification, AIDA.source, Literal("VOA_EN_NW_2015.11.26.3074625")))
        ref_graph.add((second_justification, AIDA.startOffset, Literal(14)))
        # TODO check inclusive/exclusive
        ref_graph.add((second_justification, AIDA.endOffsetInclusive, Literal(26)))
        ref_graph.add((second_justification, AIDA.system, system_node))

        second_confidence_node = node_gen.next_node()
        ref_graph.add((second_confidence_node, AIDA.confidenceValue, Literal(0.123)))
        ref_graph.add((second_confidence_node, AIDA.system, system_node))
        ref_graph.add((second_justification, AIDA.confidence, second_confidence_node))

        ref_graph.namespace_manager.bind('aida', AIDA)
