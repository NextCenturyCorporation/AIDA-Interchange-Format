...
// Only used by ColdStart2AidaInterchange.kt
/*
 class PassThroughOntologyMapper(private val model: Model,
                                override val NAMESPACE: String,
                                *//*private val*//* OntologyMapping relationsToArguments: Map<Resource, ResourcePair>)  {
    companion object {
            ...
            val relationsToArgumentAssertion = relationArgFile.bufferedReader().use {
                ...
                records.asSequence().map {
                    ResourceFactory.createResource(it.get("Relation Name")!!) to
                            new ResourcePair(ResourceFactory.createResource(it.get("Arg1")),
    ...
    override fun typeAllowedToHaveAName(type: Resource) =
            model.contains(type, RDFS.subClassOf, AidaDomainOntologiesCommon.getInstance().getCanHaveName())


    override fun typeAllowedToHaveTextValue(type: Resource) =
            model.contains(type, RDFS.subClassOf, AidaDomainOntologiesCommon.getInstance().getCanHaveTextValue())

    override fun typeAllowedToHaveNumericValue(type: Resource) =
            model.contains(type, RDFS.subClassOf, AidaDomainOntologiesCommon.getInstance().getCanHaveNumericValue())
...
public final class SeedlingOntologyMapper implements OntologyMapping {
    ...
    Set<String> entityShortNames(){ return shortNames.keys; }
...
public class RPISeedlingOntologyMapper implements OntologyMapping {
    ...
    public Resource eventArgumentType(String argName) {
        seedlingOM.eventArgumentType(argName);
    }
