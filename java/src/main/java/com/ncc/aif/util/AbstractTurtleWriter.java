 ...
 * grouped model output with comment headers in TURTLE format
 * <p>
 * Code is copy of {@link org.apache.jena.riot.writer.TurtleShell}. Would have extended
 * {@link org.apache.jena.riot.writer.TurtleShell.ShellGraph}, but it's final
 */
public abstract class AbstractTurtleWriter extends TurtleWriterBase {
    /**
     * Convenience method to write similar to RDFDataMgr
     */
    ...
    /**
     * Internal class to write TURTLE
     */
    protected abstract static class OrderedTurtleWriter$ {
        ...
        protected final Comparator<Node> labelSort;
        protected final Comparator<Node> prefixSort;

        public OrderedTurtleWriter$(IndentedWriter out, PrefixMap pmap, String baseURI, Context context) {
            this.out = out;
            if (pmap == null) {
            ...
            if (context != null && context.isTrue(RIOT.multilineLiterals)) {
                this.nodeFmt = new NodeFormatterTTL_MultiLine(baseURI, pmap, nodeToLabel);
            } else {
                this.nodeFmt = new NodeFormatterTTL(baseURI, pmap, nodeToLabel);
            }

            // Compare object representations
            labelSort = Comparator.comparing(node ->
                    node.isLiteral() ? node.getLiteral().toString(false) :
                            node.isBlank() ? nodeToLabel.get(null, node) : node.getURI());

            // Compare prefixes or uris. Used for comparing subjects
            prefixSort = Comparator.comparing(node ->
                    prefixMap.abbrev(node.getURI()) == null ? node.getURI() :
                            prefixMap.abbrev(node.getURI()).getRight());
        }

        protected void writeHeader(String headerText) {
            writeHeader(headerText, false);
        }

        protected void writeHeader(String headerText, boolean somethingWritten) {
            if (somethingWritten) {
                out.println();
            }
            out.println("######################################################\n" +
         ...
         */
        protected abstract void writeGraphTTL(Graph graph);

        /**
         * In some cases, we do not want to print out the node using the
         * formatting, because it replaces things with the prefix.  In particular,
         * we do not want it to replace the definition of the ontology.
         *
         * @param node
         */
        protected final void writeLiteralNode(Node node) {
        ...
        protected class BaseGraphWriter {
            ...
            protected final Map<Node, List<Node>> freeLists;
            ...
            protected final Collection<Node> listElts;

            // Blank nodes that have one incoming triple
            protected /*final*/ Set<Node> nestedObjects;
            ...
            protected BaseGraphWriter(Graph graph, Node graphName, DatasetGraph dsg) {
                this.dsg = dsg;
            ...
            private BaseGraphWriter(Graph graph) {
            ...
            protected Iterator<Node> listSubjects() {
            ...
            private boolean validListElement(Node x, List<Triple> acc) {
                ...
                long count = countTriples(x, null, null);
                if (count != 2) {
                    return false;
                }
                acc.add(t1);
            ...
            protected void writeRemainder(boolean somethingWritten) {
                // Write remainders
                // 1 - Shared lists
            ...
            protected boolean writeSingleSubject(Node subject, boolean somethingWritten) {
                if (nestedObjects.contains(subject) || listElts.contains(subject)) {
                    return somethingWritten;
                }
                ...
                if (freeBnodes.contains(subject)) {
                    // Top level: write in "[....]" on "[] :p" form.
                    writeNestedObjectTopLevel(subject);
                } else {
                    Collection<Triple> cluster = triplesOfSubject(subject);
                    writeCluster(subject, cluster);
                }
            ...
            protected boolean writeBySubject(Iterator<Node> subjects) {
                return writeBySubject(subjects, false);
            }

            protected boolean writeBySubject(Iterator<Node> subjects, boolean somethingWritten) {
                for (; subjects.hasNext(); ) {
                    somethingWritten = writeSingleSubject(subjects.next(), somethingWritten);
                }
                return somethingWritten;
            }
            ...
            protected void writePredicateObjectList(Node p, List<Node> objects, int predicateMaxWidth, boolean first) {
                objects.sort(labelSort);
            ...
            // Get Triples matching provided p and o
            protected ExtendedIterator<Triple> getTriplesByPO(Property p, String uri) {
                return graph.find(Node.ANY, p.asNode(), NodeFactory.createURI(uri));
            }

            protected SortedSet<Node> getSortedSubjects(ExtendedIterator<Triple> iter) {
                // create Set of Nodes sorted by local names
                SortedSet<Node> subjects = new TreeSet<>(prefixSort);

                if (iter.hasNext()) {
                    // add subjects to sorted set
                    Iter.iter(iter).map(Triple::getSubject).distinct().forEachRemaining(subjects::add);
                }

                return subjects;
            }

            protected void writeOrderedSubjects(SortedSet<Node> subjects) {
                // write subjects to output
                writeBySubject(subjects.iterator());

                // create new line for end of document and after each set of subjects written
                out.println();
            }

            protected void writeOrderedSubjectsFromTriples(ExtendedIterator<Triple> iter) {
                writeOrderedSubjects(getSortedSubjects(iter));
            }

            protected void writeOrderedSubjectsWithHeader(ExtendedIterator<Triple> iter, String header) {
                if (iter.hasNext()) {
                    writeHeader(header);
                    writeOrderedSubjectsFromTriples(iter);
                }
            }

            protected void writeOrderedSubjectsOfSubclass(String parentUri, String header) {
                ExtendedIterator<Triple> triples = getTriplesByPO(RDFS.subClassOf, parentUri);
                writeOrderedSubjectsWithHeader(triples, header);
            }
        }
    }
}
