...
from flexnlp.utils.preconditions import check_arg, check_not_none, \
    check_isinstance
from flexnlp_sandbox.formats.tac.coldstart import ColdStartKB, \
    ColdStartKBLoader, TypeAssertion, \
    Node, EntityNode, EventNode, StringNode, EntityMentionAssertion, \
    CANONICAL_MENTION, \
    LinkAssertion, EventMentionAssertion, RelationAssertion, Provenance, \
    RealisType, \
...
class ColdStartToGaiaConverter:
    ...
    def convert_coldstart_to_gaia(self, system_uri: str, cs_kb: ColdStartKB) -> Graph:
        ...
        def associate_with_system(identifier: Identifier) -> None:
            ...
            g.add((identifier, AIDA_ANNOTATION.system, system_node))
        ...
        def mark_single_assertion_confidence(reified_assertion, confidence: float) -> None:
            ...
            g.add((reified_assertion, AIDA_ANNOTATION.confidence,
                   confidence_blank_node))
            g.add((confidence_blank_node, AIDA_ANNOTATION.confidenceValue,
                   Literal(confidence)))
            g.add((confidence_blank_node, AIDA_ANNOTATION.system, system_node))
        ...
        def register_justifications(g: Graph,
            ...
            for justification in provenance.predicate_justifications:
                ...
                g.add((justification_node, RDF.type,
                       AIDA_ANNOTATION.TextProvenance))
                g.add((justification_node, AIDA_ANNOTATION.source,
                       Literal(provenance.doc_id)))
                g.add((justification_node, AIDA_ANNOTATION.startOffset,
                       Literal(justification.start)))
                # +1 because Span end is exclusive but interchange format is inclusive
                g.add((justification_node, AIDA_ANNOTATION.endOffsetInclusive,
                       Literal(justification.end + 1)))
                g.add((entity, AIDA_ANNOTATION.justifiedBy, justification_node))
        ...
        def translate_event_mention(g: Graph, cs_assertion: EventMentionAssertion,
            ...
            realis_node = BNode()
            g.add((entity_uri, AIDA_ANNOTATION.realis, realis_node))
            g.add((realis_node, AIDA_ANNOTATION.realisValue,
                   to_realis_type(cs_assertion.realis)))
        ...
        g.namespace_manager.bind('aida', AIDA_ANNOTATION)
